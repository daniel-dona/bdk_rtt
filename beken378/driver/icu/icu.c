...
uint32_t icu_ctrl(uint32_t cmd, void *param)
{
    uint32_t ret;
    uint32_t reg;
    ...
    case CMD_CLR_INTR_STATUS:
        ...
        REG_WRITE(ICU_INT_STATUS, (reg | *(uint32_t *)param));
        break;
    ...
    case CMD_CLR_INTR_RAW_STATUS:
        ...
        REG_WRITE(ICU_INT_RAW_STATUS, (reg | *(uint32_t *)param));
        break;

    case CMD_SET_JTAG_MODE:
        ASSERT(param);
        if(JTAG_ARM_MODE == (*(uint32_t *)param))
            REG_WRITE(ICU_JTAG_SELECT, JTAG_SEL_WR_ARM);
        else if(JTAG_TL410_MODE == (*(uint32_t *)param))
