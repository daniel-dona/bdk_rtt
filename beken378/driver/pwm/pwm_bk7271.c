...
void (*p_PWM_Int_Handler[PWM_CHANNEL_NUM])(uint8_t);
...
static void pwm_icu_configuration(pwm_param_t *pwm_param, uint8_t enable)
{
	uint32_t ret;
	uint32_t param;
...
static bk_err_t get_pwm_current_channel(uint8_t channel)
{
...
static void init_pwm_param(pwm_param_t *pwm_param, uint8_t enable)
{
	uint32_t value;
	...
	if (current_pwm_channel == 0) {
		...
		value = (uint32_t) pwm_param->duty_cycle;
...
static void pwm_unit_enable(uint8_t ucChannel)
{
...
void pwm_unit_disable(uint8_t ucChannel)
{
...
static void pwm_param_clear(uint8_t ucChannel)
{
	pwm_group =  get_pwm_current_channel(ucChannel);
...
static uint32_t pwm_capture_value_get(uint8_t ucChannel)
{
...
uint32_t pwm_get_counter(uint8_t ucChannel)
{
	uint32_t value;
