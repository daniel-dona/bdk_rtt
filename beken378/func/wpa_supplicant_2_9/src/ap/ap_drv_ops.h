...
uint32_t hostapd_sta_flags_to_drv(uint32_t flags);
...
int hostapd_set_wds_sta(struct hostapd_data *hapd, char *ifname_wds,
			const uint8_t *addr, int aid, int val);
int hostapd_sta_add(struct hostapd_data *hapd,
		    const uint8_t *addr, uint16_t aid, uint16_t capability,
		    const uint8_t *supp_rates, size_t supp_rates_len,
		    uint16_t listen_interval,
		    const struct ieee80211_ht_capabilities *ht_capab,
		    const struct ieee80211_vht_capabilities *vht_capab,
		    const struct ieee80211_he_capabilities *he_capab,
		    size_t he_capab_len,
		    uint32_t flags, uint8_t qosinfo, uint8_t vht_opmode, int supp_p2p_ps,
		    int set);
int hostapd_set_privacy(struct hostapd_data *hapd, int enabled);
int hostapd_set_generic_elem(struct hostapd_data *hapd, const uint8_t *elem,
			     size_t elem_len);
int hostapd_get_ssid(struct hostapd_data *hapd, uint8_t *buf, size_t len);
int hostapd_set_ssid(struct hostapd_data *hapd, const uint8_t *buf, size_t len);
...
int hostapd_get_seqnum(const char *ifname, struct hostapd_data *hapd,
		       const uint8_t *addr, int idx, uint8_t *seq);
...
int hostapd_sta_set_flags(struct hostapd_data *hapd, uint8_t *addr,
			  int total_flags, int flags_or, int flags_and);
int hostapd_sta_set_airtime_weight(struct hostapd_data *hapd, const uint8_t *addr,
				   unsigned int weight);
int hostapd_set_country(struct hostapd_data *hapd, const char *country);
int hostapd_set_tx_queue_params(struct hostapd_data *hapd, int queue, int aifs,
				int cw_min, int cw_max, int burst_time);
struct hostapd_hw_modes *
hostapd_get_hw_feature_data(struct hostapd_data *hapd, uint16_t *num_modes,
			    uint16_t *flags, uint8_t *dfs_domain);
...
int hostapd_driver_set_noa(struct hostapd_data *hapd, uint8_t count, int start,
			   int duration);
...
int hostapd_drv_send_mlme_csa(struct hostapd_data *hapd,
			      const void *msg, size_t len, int noack,
			      const uint16_t *csa_offs, size_t csa_offs_len);
int hostapd_drv_sta_deauth(struct hostapd_data *hapd,
			   const uint8_t *addr, int reason);
int hostapd_drv_sta_disassoc(struct hostapd_data *hapd,
			     const uint8_t *addr, int reason);
int hostapd_drv_send_action(struct hostapd_data *hapd, unsigned int freq,
			    unsigned int wait, const uint8_t *dst, const uint8_t *data,
			    size_t len);
int hostapd_drv_send_action_addr3_ap(struct hostapd_data *hapd,
				     unsigned int freq,
				     unsigned int wait, const uint8_t *dst,
...
int hostapd_add_sta_node(struct hostapd_data *hapd, const uint8_t *addr,
			 uint16_t auth_alg);
int hostapd_sta_auth(struct hostapd_data *hapd, const uint8_t *addr,
		     uint16_t seq, uint16_t status, const uint8_t *ie, size_t len);
int hostapd_sta_assoc(struct hostapd_data *hapd, const uint8_t *addr,
		      int reassoc, uint16_t status, const uint8_t *ie, size_t len);
int hostapd_add_tspec(struct hostapd_data *hapd, const uint8_t *addr,
		      uint8_t *tspec_ie, size_t tspec_ielen);
...
int hostapd_drv_update_dh_ie(struct hostapd_data *hapd, const uint8_t *peer,
			     uint16_t reason_code, const uint8_t *ie, size_t ielen);


#include "drivers/driver.h"

int hostapd_drv_wnm_oper(struct hostapd_data *hapd,
			 enum wnm_oper oper, const uint8_t *peer,
			 uint8_t *buf, uint16_t *buf_len);

int hostapd_drv_set_qos_map(struct hostapd_data *hapd, const uint8_t *qos_map_set,
...
static inline int hostapd_drv_set_sta_vlan(const char *ifname,
					   struct hostapd_data *hapd,
					   const uint8_t *addr, int vlan_id)
{
...
static inline int hostapd_drv_set_radius_acl_auth(struct hostapd_data *hapd,
						  const uint8_t *mac, int accepted,
...
static inline void hostapd_drv_poll_client(struct hostapd_data *hapd,
					   const uint8_t *own_addr, const uint8_t *addr,
					   int qos)
{
...
static inline int hostapd_drv_br_add_ip_neigh(struct hostapd_data *hapd,
					      int version, const uint8_t *ipaddr,
...
static inline int hostapd_drv_br_delete_ip_neigh(struct hostapd_data *hapd,
						 uint8_t version, const uint8_t *ipaddr)
{
	if (hapd->driver == NULL || hapd->drv_priv == NULL ||
