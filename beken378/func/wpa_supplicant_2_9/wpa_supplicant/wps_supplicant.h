...
int wpas_wps_start_pbc(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
		       int p2p_group, int multi_ap_backhaul_sta);
int wpas_wps_start_pin(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
		       const char *pin, int p2p_group, uint16_t dev_pw_id);
void wpas_wps_pbc_overlap(struct wpa_supplicant *wpa_s);
int wpas_wps_cancel(struct wpa_supplicant *wpa_s);
int wpas_wps_start_reg(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
...
int wpas_wps_scan_result_text(const uint8_t *ies, size_t ies_len, char *pos,
			      char *end);
int wpas_wps_er_start(struct wpa_supplicant *wpa_s, const char *filter);
void wpas_wps_er_stop(struct wpa_supplicant *wpa_s);
int wpas_wps_er_add_pin(struct wpa_supplicant *wpa_s, const uint8_t *addr,
...
int wpas_wps_start_nfc(struct wpa_supplicant *wpa_s, const uint8_t *dev_addr,
		       const uint8_t *bssid,
		       const struct wpabuf *dev_pw, uint16_t dev_pw_id,
		       int p2p_group, const uint8_t *peer_pubkey_hash,
		       const uint8_t *ssid, size_t ssid_len, int freq);
...
void wpas_wps_notify_assoc(struct wpa_supplicant *wpa_s, const uint8_t *bssid);
...
static inline uint8_t wpas_wps_get_req_type(struct wpa_ssid *ssid)
{
	return 0;
}
