...
int wpa_supplicant_ap_wps_pbc(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
			      const uint8_t *p2p_dev_addr);
int wpa_supplicant_ap_wps_pin(struct wpa_supplicant *wpa_s, const uint8_t *bssid,
...
void ap_tx_status(void *ctx, const uint8_t *addr,
		  const uint8_t *buf, size_t len, int ack);
void ap_eapol_tx_status(void *ctx, const uint8_t *dst,
			const uint8_t *data, size_t len, int ack);
void ap_client_poll_ok(void *ctx, const uint8_t *addr);
void ap_rx_from_unknown_sta(void *ctx, const uint8_t *addr, int wds);
void ap_mgmt_rx(void *ctx, struct rx_mgmt *rx_mgmt);
void ap_mgmt_tx_cb(void *ctx, const uint8_t *buf, size_t len, uint16_t stype, int ok);
...
int wpas_ap_wps_add_nfc_pw(struct wpa_supplicant *wpa_s, uint16_t pw_id,
			   const struct wpabuf *pw, const uint8_t *pubkey_hash);
...
int wpas_ap_pmksa_cache_list_mesh(struct wpa_supplicant *wpa_s, const uint8_t *addr,
				  char *buf, size_t len);
